...
const DistribuidorLayout = () => {
  ...
  const menuItems = [
    { name: 'Inicio', icon: '🏠', action: () => handleTabChange('inicio') },
    { name: 'Recubrimiento', icon: '🎨', action: () => handleTabChange('recubrimiento') },
    { name: 'Garantía', icon: '🛡️', action: () => handleTabChange('garantia') },
  ];
    ...
    <div className="flex flex-col h-screen bg-gray-50">
      <header ref={vantaRef} className="shadow-lg px-6 py-4 flex justify-between items-center z-10 relative overflow-hidden">
        ...
        <div className="relative z-10 flex justify-between items-center w-full">
          <h2 className="text-lg font-bold text-white drop-shadow-[0_2px_8px_rgba(37,99,235,0.5)]">📦 Panel Distribuidor</h2>
          <button 
            onClick={() => {
